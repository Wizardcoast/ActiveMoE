        ...
        for key in self.args.json_keys:
            ...
            for sentence in sentences:
                if self.args.fixed_length:
                    sentence_ids = Encoder.tokenizer.tokenize_as_chunk(sentence,seq_length=self.args.fixed_length)
                else:
            ...
            if len(doc_ids) > 0 and self.args.append_eod:
                if self.args.fixed_length:
                    if len(sentence_lens) > 1:
                        raise ValueError('text is split into sentences. With fixed_length set, it is not compatible.')
                    # find the first pad then set it to eod
                    first_pad_idx = None

                    for idx,tid in enumerate(doc_ids):

                        if tid == Encoder.tokenizer.pad_token_id:
                            first_pad_idx = idx
                            break

                    if first_pad_idx is not None:
                        doc_ids[first_pad_idx] = Encoder.tokenizer.eod
                else:
                    doc_ids.append(Encoder.tokenizer.eod)
                    # for pad it is swap so no need to add 1
                    sentence_lens[-1] += 1
...
def get_args():
    ...
    group.add_argument('--fixed-length',  type=int, default=None,
                       help='pad mode. pad or truncate to fixed-length')
    ...
    if args.fixed_length and args.split_sentences:
        raise ValueError('pad or truncate to fixed length is in conflict with split sentences')
