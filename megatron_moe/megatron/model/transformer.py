...
class ParallelMLP(MegatronModule):
    ...
    def __init__(self, config, is_expert=False, share_expert_ffn_hidden_size=None):
        super(ParallelMLP, self).__init__()
        args = get_args()

        self.add_bias = config.add_bias_linear
        if share_expert_ffn_hidden_size == None:
            ffn_hidden_size = config.ffn_hidden_size
        else:
            ffn_hidden_size = share_expert_ffn_hidden_size
...
class CoreAttention(MegatronModule):
    ...
    def __init__(self, layer_number, config,
        ...
        args = get_args()
        # if reset_attention_mask, should not used upper triangle mask.
        custom_mask = args.reset_attention_mask
...
class ParallelAttention(MegatronModule):
    ...
    def __init__(self, config, layer_number,
        ...
        if attention_type == AttnType.self_attn:
            self.query_key_value = tensor_parallel.ColumnParallelLinear(
                ...
                config=config,
                init_method=config.init_method,
                bias=args.add_bias_linear or args.bias_attn_linear,
                gather_output=False)
        else:
            ...
            self.query = tensor_parallel.ColumnParallelLinear(
                ...
                config=config,
                init_method=config.init_method,
                bias=config.add_bias_linear or args.bias_attn_linear,
                gather_output=False)
    ...
    def forward(self, hidden_states, attention_mask,
        ...
        if not self.use_flash_attn and \
...
class ParallelTransformerLayer(MegatronModule):
    ...
    def __init__(self, config,
        ...
        '''
        if no_moe == False:
            # adapt to Moe and dMoe
            if args.moe_num_experts is not None:
                if args.moe_use_megatron_switch:
                    self.mlp = SwitchMLP(config)
                elif args.moe_capacity_factor > 0:
                    self.mlp = MoE(config.init_method, config.output_layer_init_method)
                    if args.add_moe_share_expert == True:
                        self.mlp_share = ParallelMLP(config, args.moe_share_expert_ffn_hidden_size)
                else:
    ...
    def forward(self, hidden_states, attention_mask,
        ...
        # share expert MLP
        if self.args.add_moe_share_expert == True and self.no_moe == False:
            mlp_share_output, mlp_share_bias = self.mlp_share(norm_output)
            mlp_output = mlp_output * self.args.moe_weight + mlp_share_output * self.args.share_weight
            if mlp_bias is not None and mlp_share_bias is not None:
                mlp_bias = mlp_bias * self.args.moe_weight + mlp_share_bias * self.args.share_weight
        print('-------------------------')
        print('mlp')
        print(mlp_output)
...
class ParallelTransformer(MegatronModule):
    ...
    def __init__(self, config,
        ...
        def build_layer(layer_number, no_moe=False):
        ...
        if self.num_layers == 0:
            ...
            assert args.num_layers >= args.moe_num_layers
            mlp_num_layers = args.num_layers - args.moe_num_layers
            
            if self.num_layers + offset <= mlp_num_layers:
                self.layers = torch.nn.ModuleList(
                    [build_layer(i + 1 + offset, no_moe=True) for i in range(self.num_layers)])
            elif  offset < mlp_num_layers and offset + self.num_layers > mlp_num_layers:
                self.layers = torch.nn.ModuleList(
                    [build_layer(i + 1 + offset, no_moe=True) for i in range(mlp_num_layers-offset)]+
                    [build_layer(i + 1 + mlp_num_layers) for i in range(offset+self.num_layers-mlp_num_layers)])
            else:
                self.layers = torch.nn.ModuleList(
                    [build_layer(i + 1 + offset) for i in range(self.num_layers)])
            
            '''
            if self.num_layers == args.moe_num_layers:
                self.layers = torch.nn.ModuleList(
                    [build_layer(i + 1 + offset) for i in range(self.num_layers)])
            else:

                self.layers = torch.nn.ModuleList(
                    [build_layer(i + 1 + offset, no_moe=True) for i in range(mlp_num_layers)]+
                    [build_layer(i + 1 + offset+mlp_num_layers) for i in range(args.moe_num_layers)])
            '''
    ...
    def forward(self, hidden_states, attention_mask,
        ...
        print('==================================================')
        print('hidden_states before final norm')
        print(hidden_states.size())
        print(hidden_states)
        print('==================================================')
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        for key in state_dict.keys():
            # Handle renaming layernorm -> norm in component names for checkpoint compatibility.
            newkey = key.replace("layernorm", "norm")
            if 'self_attention.query_key_value' not in key or not self.args.num_query_groups:
                state_dict_[newkey] = state_dict[key]
                continue

            world_size = mpu.get_tensor_model_parallel_world_size()
            num_attention_heads_per_partition = core.utils.divide(self.config.num_attention_heads, world_size)
            query_projection_size = self.config.kv_channels * num_attention_heads_per_partition
            param = state_dict[key]
            last_dim = self.config.hidden_size
            num_qgs_per_partition = core.utils.divide(self.args.num_query_groups, world_size)
            assert param.size(0) == 3 * query_projection_size, "Only support resuming from MHA checkpoints."

            if len(param.size()) == 1:
                param = param.view(-1, 1)
                last_dim = 1

            param = param.view(-1, 3*self.config.kv_channels, last_dim)
            query, key, value = torch.split(param, self.config.kv_channels, dim=1)
            query = query.view(num_qgs_per_partition, -1, self.config.kv_channels, last_dim)
            key = key.view(num_qgs_per_partition, -1, self.config.kv_channels, last_dim)
            key = key.mean(dim=1, keepdim=True)
            value = value.view(num_qgs_per_partition, -1, self.config.kv_channels, last_dim)
            value = value.mean(dim=1, keepdim=True)
            gq_param = torch.cat((query, key, value), dim=1).view(-1, self.config.kv_channels, last_dim)
            gq_param = gq_param.view(-1, last_dim)
            if last_dim == 1:
                gq_param = gq_param.squeeze(-1)

            state_dict_[newkey] = gq_param
