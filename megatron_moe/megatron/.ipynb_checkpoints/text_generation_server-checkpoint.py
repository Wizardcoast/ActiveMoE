...
class MegatronGenerate_local(Resource):
    def __init__(self, model):
        self.model = model
    ...
    def put(self, input):
        ...
        if not "prompts" in input:
            return "prompts argument required", 400

        if "max_len" in input:
            return "max_len is no longer used.  Replace with tokens_to_generate", 400

        if "sentences" in input:
            return "sentences is no longer used.  Replace with prompts", 400
        ...
        tokens_to_generate = 64  # Choosing hopefully sane default.  Full sequence is slow
        if "tokens_to_generate" in input:
            tokens_to_generate = input["tokens_to_generate"]
            if not isinstance(tokens_to_generate, int):
                return "tokens_to_generate must be an integer greater than 0"
            if tokens_to_generate < 0:
                return "tokens_to_generate must be an integer greater than or equal to 0"
        ...
        top_p_decay = 0.0
        if "top_p_decay" in input:
            top_p_decay = input["top_p_decay"]
            if not (type(top_p_decay) == float):
                return "top_p_decay must be a positive float less than or equal to 1.0"
            if top_p == 0.0:
                return "top_p_decay cannot be set without top_p"
        ...
        top_p_bound = 0.0
        if "top_p_bound" in input:
            top_p_bound = input["top_p_bound"]
            if not (type(top_p_bound) == float):
                return "top_p_bound must be a positive float less than or equal to top_p"
            if top_p == 0.0:
                return "top_p_bound cannot be set without top_p"
        ...
        stop_on_double_eol = False
        if "stop_on_double_eol" in input:
            stop_on_double_eol = input["stop_on_double_eol"]
            if not isinstance(stop_on_double_eol, bool):
                return "stop_on_double_eol must be a boolean value"

        stop_on_eol = False
        if "stop_on_eol" in input:
            stop_on_eol = input["stop_on_eol"]
            if not isinstance(stop_on_eol, bool):
                return "stop_on_eol must be a boolean value"

        prevent_newline_after_colon = False
        if "prevent_newline_after_colon" in input:
            prevent_newline_after_colon = input["prevent_newline_after_colon"]
            if not isinstance(prevent_newline_after_colon, bool):
                return "prevent_newline_after_colon must be a boolean value"
        ...
        with lock:  # Need to get lock to keep multiple threads from hitting code
            ...
            try:
                if beam_width is not None:
                    MegatronGenerate_local.send_do_beam_search()  # Tell other ranks we're doing beam_search
                    response, response_seg, response_scores = \
                        beam_search_and_post_process(
                        self.model,
                        prompts=prompts,
                    ...
                    MegatronGenerate_local.send_do_generate()  # Tell other ranks we're doing generate
                    response, response_seg, response_logprobs, _ = \
                        generate_and_post_process(
                        self.model,
                        prompts=prompts,
