...
def _should_shuffle(data_prefix):
    args = get_args()

    if args.no_shuffle_dataset is None:
        return True
    shuffle=True
    # if in no_shuffle_dataset, then no shuffle
    # 注意：shuffle这个参数没有作为cache key的一部分，也就是说如果其他参数一样，只改变shuffle
    # idx不会重新变化，而会直接走cache，这是时候手动rm cache即可。
    for no_shuffle_dataset in args.no_shuffle_dataset:
        if data_prefix.endswith(no_shuffle_dataset):
            shuffle=False
            break
...
def _build_train_valid_test_datasets(data_prefix, splits_string,
    ...
    # 这个build dataset是针对传一个data_path，即我们的常用模式. 另外的模式是分别传train、valid、test
    def build_dataset(index, name):
        dataset = None
        shuffle = _should_shuffle(data_prefix)
...
def _build_dataset(dataset_name, data_prefix, splits_string,
    ...
    dataset = GPTDataset(dataset_name, data_prefix, documents, indexed_dataset,
                         ...
                         data_cache_path=data_cache_path, shuffle=shuffle)
...
class GPTDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, name, data_prefix, documents, indexed_dataset,
                 splits_string, num_samples, seq_length, seed,
                 return_doc_ids=False, *,
                 data_cache_path=None, shuffle=True):
...
def _build_index_mappings(name, data_prefix, documents, sizes,
    ...
    counts = torch.tensor([data_cache_success], dtype=torch.long, device='cuda')
    if get_args().tp_shared_dataloader:
