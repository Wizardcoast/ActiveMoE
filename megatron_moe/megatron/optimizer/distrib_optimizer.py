    ...
    def build_model_and_main_param_groups(cls,
        ...
        for group_index, group_range in enumerate(opt_group_ranges):
            ...
            for model_param in group_range["params"]:
                ...
                # fp16, bf16 params.
                if model_param.dtype in [torch.half, torch.bfloat16]:
                ...
                # fp32 params.
                    ...
                    raise TypeError('Dtype of wrapped parameters must be one of '
                                    'torch.float32, torch.half, torch.bfloat16. '
                                    'Received {}'.format(model_param.dtype))
    ...
    def __init__(self, optimizer, clip_grad, log_num_zeros_in_grad,
        ...
        for _, model_chunk in enumerate(self.models):
            self.per_bucket_numel.append(
                {dtype: [bucket.true_numel for bucket in model_chunk.grad_buffers[dtype].buckets]
                    ...
                    for key, recv_tensor in local_shards.items():
                        ...
                        if data_parallel_rank == 0:
                            ...
                            if world_tensor.numel() < gbuf_world_numel:
                                pad_zeros = torch.zeros(gbuf_world_numel-world_tensor.numel(),
                                                        device=world_tensor.device,
                                                        dtype=world_tensor.dtype,
                                                        requires_grad=False)
                                world_tensor = torch.cat((world_tensor, pad_zeros))
                            elif world_tensor.numel() > gbuf_world_numel:
