...
def validate_args(args, defaults={}):
    ...
    # Overlapping grad reduce not supported with Tie embeddings right now.
    if args.overlap_grad_reduce:
        assert args.untie_embeddings_and_output_weights, 'Do not support the case of tie embeddings.'
    ...
    if args.ffn_hidden_size is None:
        if args.swiglu:
            ...
            _swiglu_divisible_by = args.swiglu_make_ffn_hidden_size_divisible_by
            if _swiglu_divisible_by > 0:
                _tmp = 4 * args.hidden_size
                _tmp = int(2 * _tmp / 3)
                args.ffn_hidden_size = \
                    _swiglu_divisible_by * ((_tmp + _swiglu_divisible_by - 1) // _swiglu_divisible_by)
        else:
    ...
    if args.num_query_groups is not None:
        assert 0 < args.num_query_groups < args.num_attention_heads, \
            "Num Query Groups must be valid at (0, num_attn_heads)"
...
def _add_network_size_args(parser):
    ...
    group.add_argument('--num-query-groups', type=int, default=None,
                       help='Default None means using multi-head attention.')
    ...
    group.add_argument('--swiglu-make-ffn-hidden-size-divisible-by', type=int, default=256,
                       help='Pad the ffn-hidden-size to be divisible by this value.'
...
def _add_training_args(parser):
    ...
    group.add_argument('--clone-scatter-output-in-embedding', action='store_true',
                       help='If set, clone the output of the scatter in embedding layer to GC original tensor.')
    group.add_argument('--profile', action='store_true',
                       help='Enable kineto profiling.')
    group.add_argument('--profile-save-dir', type=str, default=None,
                       help='Directory for the saving of tracing files. Default not to save.')
    group.add_argument('--profile-ranks', nargs='+', type=int, default=None,
                       help='Global ranks to profile.')
    group.add_argument('--profile-schedule', nargs='+', type=int, default=[2, 1, 1, 1],
                       help='Schedule list for kineto profiling.')
    ...
    group.add_argument('--profile-with-flops', action='store_true',
                       help='use formula to estimate the FLOPs (floating point operations) of specific '
                       'operators(matrix multiplication and 2D convolution)')
    group.add_argument('--profile-record-shapes', action='store_true',
                       help='save information about input shapes of the operator.')
    ...
    group.add_argument('--keep-bias-attn-linear', action='store_true',
                       help='Keep bias in the attention linear layer (Wq, Wk, Wv), '
                       'when --disble-bias-linear is enabled.',
                       dest='bias_attn_linear')
...
def _add_initialization_args(parser):
    ...
    group.add_argument('--disable-scaled-init-method', action='store_true',
                       help='Disable scaled initialization related to the number of layers.')
...
def _add_checkpointing_args(parser):
    ...
    group.add_argument('--no-load-lr-scheduler', action='store_true', default=None,
                       help='Do not load learning-rate scheduler when loading checkpoint.')
    group.add_argument('--no-load-rng', action='store_true', default=None,
                       help='Do not load rng state when loading checkpoint.')
    group.add_argument('--reset-sample-and-iteration-stat', action='store_true', default=None,
                       help='Reset the consumed sample stat when loading checkpoint.')
    ...
    group.add_argument('--use-checkpoint-args', action='store_true',
                       ...
                       'from the checkpoint')
    group.add_argument('--continue-on-missing-checkpoint', action='store_true',
                       help="If '--load' is set, but checkpoint is not found "
                       "(e.g., path typo), then exit instead of continuing with"
                       "random initialization.")
    group.add_argument('--dense2moe', action='store_true',
                       help="Transform a dense checkpoint to moe model in checkpoint loading.")
...
def _add_distributed_args(parser):
    ...
    group.add_argument('--bucket-size', type=int, default=40000000,
                       help='Bucket size for ovelapping gradients all-reduce.')
    ...
    group.add_argument('--local_rank', '--local-rank', type=int, default=None,
...
def _add_data_args(parser):
    ...
    group.add_argument('--no-shuffle-dataset', nargs='*', default=None,
                       help='list of dataset that will not be shuffled. ')
    ...
    group.add_argument('--disable-tp-shared-dataloader', action='store_false',
                       help='Use shared dataloader, broadcast data from tp-rank 0.',
                       dest='tp_shared_dataloader')
    ...
    group.add_argument('--sft-loss-weight-factor', type=float,default=0.01,
                       help='loss weight factor used only for sft. by default 0.01 so max loss weight in data is 100')
...
def _add_moe_args(parser):
    ...
    group.add_argument('--mlp_type', type=str,
                       default='glu', choices=['mlp','glu','glu_llama'], help='Change the mlp type in Moe'
                       ' Containing mlp, glu, glu_llama')
    group.add_argument('--add_moe_bias', action='store_true', default=False,
                       help='add the bias to moe layer')
    group.add_argument('--moe_num_layers', type=int, default=24,
                       help='the number of layers using Moe, if it is smaller than num_layers, the last moe_num_layers layers use Moe')
    group.add_argument('--add_moe_share_expert', action='store_true', default=False,
                       help='add share expert parallel with the moe')
    group.add_argument('--share_weight', type=float, default=1,
                       help='the weight of share expert, share_weight*share+moe_weight*moe')
    group.add_argument('--moe_weight', type=float, default=1,
                       help='the weight of moe, share_weight*share+moe_weight*moe')
    group.add_argument('--moe_share_expert_ffn_hidden_size', type=int, default=None,
                       help='ffn hidden size in moe share expert')
    group.add_argument('--moe_expert_ffn_hidden_size', type=int, default=None,
                       help='ffn hidden size in moe fine-grained expert')
    return parser