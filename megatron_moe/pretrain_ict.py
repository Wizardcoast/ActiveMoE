...
def loss_func(output_tensor):
    ...
    def topk_accuracy(k):
        return torch.tensor([sum([int(i in sorted_indices[i, :k]) \
            for i in range(global_batch_size)]) / global_batch_size], dtype=torch.float, device='cuda')
...
def forward_step(data_iterator, model):
    ...
    query_types = torch.empty(*query_tokens.shape, dtype=torch.long, device='cuda').fill_(0)
    context_types = torch.empty(*context_tokens.shape, dtype=torch.long, device='cuda').fill_(0)
