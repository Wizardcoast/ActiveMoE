...
class PhiAttention(nn.Module):
    ...
    def __init__(self, config: MoPhiConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
class MoPhiDecoderLayer(nn.Module):
    def __init__(self, config: MoPhiConfig, layer_idx: int):
        super().__init__()
        self.self_attn = PHI_ATTENTION_CLASSES[config._attn_implementation](config, layer_idx=layer_idx)

        self.is_moe_layer = config.enable_moe and (layer_idx >= config.moe_start_layer and (
                    layer_idx - config.moe_start_layer) % config.moe_layer_freq == 0)
        self.config = config
        self.layer_idx = layer_idx
        if self.is_moe_layer and not config.first_init:
            print("MoLlamaDecoderLayer: using MoE at layer {}".format(layer_idx))
            self.mlp = MoPhiFFNs(
            ...
            if self.is_moe_layer and config.first_init:
                print(f"skip layer {layer_idx} MoE init due to first_init")
    ...
    def forward(
        ...
        if self.is_moe_layer:
            mlp_states, aux_loss = self.mlp(hidden_states)
            # need to accumulate aux_loss from all layers!
            self.aux_loss = aux_loss
        else:
            mlp_states = self.mlp(hidden_states)

        feed_forward_hidden_states = self.resid_dropout(mlp_states)
    ...
    def re_init_mlp(self):
        config = self.config
        if self.is_moe_layer and config.first_init:
            if is_deepspeed_zero3_enabled():
                import deepspeed
                with deepspeed.zero.GatheredParameters([self.mlp.gate_proj.weight,
                    ...
                    for k in mlp_state_dict.keys():
                        mlp_state_dict[k] = mlp_state_dict[k].t().unsqueeze(0).repeat(config.num_experts, 1, 1)
                    MoE.load_state_dict(mlp_state_dict, strict=False)
                    self.mlp = MoE
                    print("re-init MoE at layer {}".format(self.layer_idx))
            else:
                MoE = MoPhiFFNs(
                    input_size=config.hidden_size,
                    head_size=config.intermediate_size,
...
class MoPhiPreTrainedModel(PreTrainedModel):
    config_class = MoPhiConfig
...
class MoPhiModel(MoPhiPreTrainedModel):
    """
    ...
    def __init__(self, config: MoPhiConfig):
        ...
        self.layers = nn.ModuleList(
            [MoPhiDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        for decoder_layer in self.layers:
            ...
            if decoder_layer.aux_loss is not None:
                self.aux_loss += decoder_layer.aux_loss
...
class MoPhiForCausalLM(MoPhiPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    # Copied from transformers.models.llama.modeling_llama.LlamaForCausalLM.__init__ with Llama->Phi,bias=False->bias=True
    def __init__(self, config):
        super().__init__(config)
        self.model = MoPhiModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        if self.config.aux_loss_weight:
            loss += self.config.aux_loss_weight + self.model.aux_loss
...
class MoPhiForSequenceClassification(MoPhiPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class MoPhiForTokenClassification(MoPhiPreTrainedModel):
    def __init__(self, config: MoPhiConfig):
        super().__init__(config)
        self.num_labels = config.num_labels
