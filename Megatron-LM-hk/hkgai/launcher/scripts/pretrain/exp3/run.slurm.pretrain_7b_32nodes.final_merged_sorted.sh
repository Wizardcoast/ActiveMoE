...
srun -l --ntasks-per-node=1 bash -c "docker run --net=host --ipc=host --rm --user $(id -u):$(id -g) --gpus all --shm-size=1024g --ulimit memlock=-1 --privileged \
     ...
     -e DATA_PATH=/workspace/megatron/slimpajama-icp-final_merged_sorted-8node_text_document \
     -e DATA_CACHE_PATH=/workspace/megatron/slimpajama-icp-final_merged_sorted-8node_text_document \
