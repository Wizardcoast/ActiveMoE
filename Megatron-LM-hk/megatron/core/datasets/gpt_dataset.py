        ...
        if not cache_hit and torch.distributed.get_rank() == 0:
            ...
            if separate_final_epoch:
                shuffle_index = _build_shuffle_index(
                    num_samples_sans_final_epoch, sample_index.shape[0] - 1, numpy_random_state, self.config.enable_shuffle
                )
            else:
                shuffle_index = _build_shuffle_index(
                    sample_index.shape[0] - 1, sample_index.shape[0] - 1, numpy_random_state, self.config.enable_shuffle
                )
